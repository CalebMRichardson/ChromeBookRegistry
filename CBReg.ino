...
#define device_version 72 // Change to the ChromeOS version you expect to use with Centipede; Changes have been reported in the following ranges 58-68, 69, 70

#define wifi_name "wifi_name" // Define SSID for your wireless connection.
#define wifi_pass "wifi_pass" // Define the password for your wireless connection.
#define wifi_security 0 //[0,1,2] Set to 0 for open, 1 for WEP, 2 for WPA
#define username "enrollment_username@example.com" // Define the user name for enrolling the device.
#define password "enrollment_pass" // The password for the enrollment GAFE account.
...
#define languageIsSelectedOnBoot 1 //[0,1] Set to 1 if the language box is selected when you first power on the device
...
#define update_via_enrollment_account 1 // [0,1] Set to 1 to update chrome via the enrollment account and navigate to chrome://chrome and check for updates
...
#define sign_in 1 //[0,1] Set to 1 to sign-in to the device after enrollment - NOTE: Will not sign-in if update_via_guest or powerwash is set to true;
...
#define update_wait_time 30 // Set to seconds to wait for Update with update_via_guest before exiting guest mode.  Update will continue while device is online.
...
void loop() { // Main Function - workflow is called within loop();
  ...
  if (digitalRead(buttonPin) == 1 ) { // Check for debugging. If not debugging, run the program
    ...
    TXLED0;
    if (update_via_guest && !update_via_enrollment_account){
    ...
    if (sign_in && skipAssetIdScreen){ // Do not sign-in if "skipAssetIdScreen" is false
      ...
      wait(15);
      enterCredentials();
      wait(30); // Wait for profile to load
      ...
      if (update_via_enrollment_account && !update_via_guest) {
        updateViaEnrollmentAccount(); // Run updates using the Enrollment account
      }
    }
    else if (skipAssetIdScreen) {
      shutDown();
    }
    showSuccess();
  }
...
void updateViaEnrollmentAccount() {
  wait(3);
  newTab();
